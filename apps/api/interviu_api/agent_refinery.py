...
* grounded sub-agent recommendations (each with its own ``.md`` definition).
...
# Competency -> sub-agent template. ``sprite`` is the frontend sprite class
...
def build_agent_spec(
    ...
    if not held:
        ...
        headline = (
            ...
            + (
                f"Delegate to {len(sub_agents)} optional sub-agent(s) to scale."
                if sub_agents
                else "No sub-agents required for current scope."
            )
        )
    elif recommended:
        ...
        if gap_competencies:
            headline = (
                ...
                f"delegate to {len(recommended)} focused sub-agent(s)."
            )
        else:
            headline = (
                f"{candidate.name} passed its competencies but needs {len(recommended)} "
                "sub-agent(s) to close trace/generalization gaps before shipping."
            )
    else:
        readiness = "refine"
...
def _render_agent_markdown(
    ...
    readiness_label = {
        "ready": "READY",
        "refine": "REFINE",
        "needs_subagents": "NEEDS SUB-AGENTS",
    }[readiness]
    lines: list[str] = [
        f"# {candidate.name} — Refined Agent Spec",
        "",
    ...
    if sub_agents:
        ...
        lines.append("- No sub-agents required for current scope.")
